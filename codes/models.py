...
import codes.metrics as metrics

from .prediction import BipartiteEdgePredLayer
from .aggregators import GCNAggregator, MeanAggregator, MaxPoolingAggregator, MeanPoolingAggregator, SeqAggregator
from .aggregators import BatchGCNAggregator
from .aggregators import AttentionAggregator, GINAggregator, GAINAggregator
...
# SAGEInfo is a named tuple that specifies the parameters
...
class SampleAndAggregate(GeneralizedModel):
    ...
    def __init__(self, placeholders, features, adj, degrees,
            ...
            model_size="small", identity_dim=0, dfs_walks=None,
            **kwargs):
        '''
        Args:
        ...
        elif aggregator_type == "batch_gcn":
            self.aggregator_cls = BatchGCNAggregator
    ...
    def sample(self, inputs, layer_infos, batch_size=None):
        ...
        Args:
            inputs: batch inputs. Node ids for a particular layer.
            batch_size: the number of inputs (different for batch inputs and negative samples).

        Returns:
            samples : starting from most deep layer, samples for each layer. Each element is of the shape [num_samples_in_layer * batch_size]
            support_sizes : starting from most deep layer, number of samples/nodes to keep at each layer
        """
        ...
        support_size = 1 # Number of nodes to sample at each iteration (depth) for aggregation per node in minibatch
        # Support size decreases as layer depth increases
        support_sizes = [support_size]
        for k in range(len(layer_infos)):
            t = len(layer_infos) - k - 1 # Flip indices to iterate from K...1
            support_size *= layer_infos[t].num_samples
            sampler = layer_infos[t].neigh_sampler
            # samples[k] is an array of size [batch_size], basically storing node ids in the batch
            # Sample num_samples number of nodes for each batch node
            node = sampler((samples[k], layer_infos[t].num_samples)) # node is an array of [batch_size, support_size]
            # node stores the neighbors for each node in the batch
            samples.append(tf.reshape(node, [support_size * batch_size,])) # Flatten out array to keep only node_ids
    ...
    def aggregate(self, samples, input_features, dims, num_samples, support_sizes, batch_size=None,
        ...
        # Obtain initial hidden representations
        hidden = [tf.nn.embedding_lookup(input_features, node_samples) for node_samples in samples]
        new_agg = aggregators is None

        # By default, create new aggregators
        if new_agg:
            aggregators = []

        # Define aggregator functions for each layer, hops = len(num_samples) = 2
        for layer in range(len(num_samples)):

            # Start when aggregarors is None
            if new_agg:

                # To account for concatenation of self and neighbors' features
                dim_mult = 2 if concat and (layer != 0) else 1

                if layer == 0:  # added for gates but not used in this implementation
                    aggregator = self.aggregator_cls(dim_mult*dims[layer], dims[layer+1], act=lambda x: x,
                                                     dropout=self.placeholders['dropout'],
                                                     name=name, concat=concat, model_size=model_size)

                elif layer == len(num_samples) - 1:
                    aggregator = self.aggregator_cls(dim_mult*dims[layer], dims[layer+1], act=lambda x: x,
                                                     dropout=self.placeholders['dropout'],
                                                     name=name, concat=concat, model_size=model_size)
            ...
            # simply because the number of neighbor nodes increases
            for hop in range(len(num_samples) - layer):
                ...
                # Debugged: Pass self_vecs and neighbor_vecs
                # ## TODO : This is not making sense ###
                # The neighbors must have smaller number of nodes compared to current layers #
    ...
    def _build(self):
        # Labels are the co-occurring nodes for each node (1 per node)
        labels = tf.reshape(
                tf.cast(self.placeholders['batch2'], dtype=tf.int64),
                [self.batch_size, 1])

        # Create negative samples for each node
        self.neg_samples, _, _ = (tf.nn.fixed_unigram_candidate_sampler(
            true_classes=labels,
            num_true=1, # There is only one node co-occuring as a truth
        ...
        # aggregate local neighbors of HOP-1
        self.outputs1, self.aggregators = self.aggregate(samples1, [self.features], self.dims, num_samples,
                                                         support_sizes1, concat=self.concat,
                                                         model_size=self.model_size)

        # aggregate local neighbors of HOP-2
        ...
        # aggregate local negative neighbors
        ...
        self.link_pred_layer = BipartiteEdgePredLayer(dim_mult*self.dims[-1], dim_mult*self.dims[-1],
                                                      self.placeholders, act=tf.nn.sigmoid, bilinear_weights=False,
                                                      name='edge_predict')
    ...
    def build(self):
        ...
        """
        Loss computation :
        Take nodes of batch1 and their ground truth as batch2
        Positive sigmoid-cross-entropy for their affinities
        Similarly the converse for the negative samples (along with inputs1)
        The result is the loss per batch
        """
        self._loss()
        self._accuracy()
