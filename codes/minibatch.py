...
class EdgeMinibatchIterator(object):
    ...
    def __init__(self, G, id2idx, 
        ...
        # Adjacency of train nodes
        self.adj, self.deg = self.construct_adj()
        # Adjacency of all nodes
        self.test_adj = self.construct_test_adj()

        if context_pairs is None:
            # Use directly graph edges
            edges = G.edges()
        else:
            # Use random-walk co-occurrences of train nodes (see utils.py)
            edges = context_pairs
            ''
        self.train_edges = self.edges = np.random.permutation(edges)
        if not n2v_retrain:
            self.train_edges = self._remove_isolated(self.train_edges)  # Remove isolated and anomaly type nodes
    ...
    def _remove_isolated(self, edge_list):
        """
        Removes isolated and anamalous edges
        """
        new_edge_list = []
        missing = 0
    ...
    def construct_adj(self):
        """
        For every node, this creates a uniform sample of it's neighbors. 
        The number of neighbors sampled is equal to the maximum degree of a node.
        It returns an array of size (V+1, max_degree_in_graph) where each row represents a node's uniformly sampled neighbors
        It also returns the degree of each node in the graph through an aray
        """
    ...
    def construct_test_adj(self):
        ...
        adj = len(self.id2idx)*np.ones((len(self.id2idx)+1, self.max_degree)) # (V+1, D)
